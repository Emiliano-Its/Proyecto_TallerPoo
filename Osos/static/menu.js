...
//PARA LOS LIKES Y DISLIKES DE LOS COMENTARIOS
function votar(elemento, tipo, idResena) {
    fetch('/votar', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            const likeSpan = elemento.parentElement.querySelector('.like-count');
            const dislikeSpan = elemento.parentElement.querySelector('.dislike-count');
